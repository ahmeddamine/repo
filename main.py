...
class VideoLimites(Scene):
    ...
    def scene_2_notion_intuitive(self):
        ...
        phrase = Text(
            "Quand x s'approche de 2, f(x) s'approche de L.",
            font_size=28,
            color=WHITE,
    ...
    def scene_4_trou(self):
        title, _ = self.title_block("Cas du trou: limite existe mais f(a) non définie", "Exemple: (x^2-1)/(x-1), Df: x != 1")
        ...
        phrase = Text(
            "La limite peut exister même si f(1) n'existe pas.",
            font_size=26,
            color=WHITE,
        ).next_to(df, DOWN, aligned_edge=LEFT)
    ...
    def scene_5_definition_bac(self):
        ...
        left_right = Text("Rappel: vérifier la gauche et la droite.", font_size=30, color=BLUE)
    ...
    def scene_11_pieges(self):
        ...
        cards = [
            Text("Piège 1: division par 0", font_size=40, color=RED),
            Text("Piège 2: confondre limite et f(a)", font_size=36, color=ORANGE),
            Text("Piège 3: oublier la condition lim g ≠ 0", font_size=34, color=RED_D),
        ]
    ...
    def scene_13_resume(self):
        ...
        recap = VGroup(
            MathTex(r"\bullet\ \lim_{x\to a}f(x)=L", font_size=30),
            Text("• Comparer limite à gauche et limite à droite", font_size=28),
            Text("• Propriétés: somme, différence, kf, produit, quotient", font_size=28),
            Text("• Quotient: condition lim g ≠ 0", font_size=28),
            Text("• Forme 0/0: factoriser puis simplifier", font_size=28),
            Text("• Racines: rationaliser avec le conjugué", font_size=28),
            Text("• Vers ±∞: comparer les degrés", font_size=28),
            Text("• Toujours vérifier Df et les conditions", font_size=28, color=YELLOW),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.16).scale(0.92).next_to(title, DOWN, buff=0.35)
